...
c_Primitive::c_Primitive(LPCSTR fzName)
{
	Reset_Primitive();
	m_pszFName		= fzName;
}
void	c_Primitive::SetFileName(LPCSTR fzName)
{
	m_pszFName = fzName;
}
c_Primitive::~c_Primitive()
...
void c_Primitive::Reset_Primitive(void)
{
	D3DV_INIT(m_vPos);
	D3DV_INIT(m_vRot);
...
//Primitiveの基本処理
//===============================================================================
HRESULT c_Primitive::Init_Primitive()
{
...
void c_Primitive::Uninit_Primitive(void)
{
...
void c_Primitive::Update_PrimitiveNormal(void)
{
...
void c_Primitive::SetWatchCamera(bool input)
{
	m_bWatchCamera = input;
}
void c_Primitive::SetPos(D3DXVECTOR3 Pos)
{
	m_vPos = Pos;
}
void c_Primitive::SetRot(D3DXVECTOR3 Rot)
{
	m_vRot = Rot;
}
void c_Primitive::SetAlpha(float fAlpha)
{	
	this->m_fAlpha = fAlpha;
}
void c_Primitive::SetReversal(bool bReversa)
{
	m_bUReversal = bReversa;
}

void c_Primitive::SetOblique(D3DXVECTOR3* input)
{
	m_vOblique[0] = input[0];
	m_vOblique[1] = input[1];
}
void c_Primitive::Update_PrimitiveMatrixOnly(void)
{
	D3DXMATRIX	matRotate;		// 回転マトリックス
	D3DXMATRIX	matTrans;		// 平行移動マトリックス
	D3DXMATRIX*	pmatInv;
...
void c_Primitive::SetVerTexBuff(void)
{
...
void c_Primitive::SetVerTexBuffSizeOblique(void)
{
	DVERTEX_PRIMITIVE*	VBuffer;								// 頂点バッファへのポインタ	// 頂点バッファに値をセット
	if (FAILED(m_pD3DVertexBuffer->Lock(0, 0, (LPVOID*)&VBuffer, 0))) 
	{		
...
void c_Primitive::SetVerTexBuffSize(void)
{
	DVERTEX_PRIMITIVE*	VBuffer;								// 頂点バッファへのポインタ	// 頂点バッファに値をセット
	if (FAILED(m_pD3DVertexBuffer->Lock(0, 0, (LPVOID*)&VBuffer, 0))) 
	{		
...
void c_Primitive::SetVerTexBuffUV(void)
{
	DVERTEX_PRIMITIVE*	VBuffer;								// 頂点バッファへのポインタ	// 頂点バッファに値をセット
	if (FAILED(m_pD3DVertexBuffer->Lock(0, 0, (LPVOID*)&VBuffer, 0))) 
	{		
...
void c_Primitive::SetVerTexBuffUVReversa(void)
{
	DVERTEX_PRIMITIVE*	VBuffer;								// 頂点バッファへのポインタ	// 頂点バッファに値をセット
	if (FAILED(m_pD3DVertexBuffer->Lock(0, 0, (LPVOID*)&VBuffer, 0))) 
	{		
...
void c_Primitive::SetVerTexBuffAlpha(void)
{
	DVERTEX_PRIMITIVE*	VBuffer;								// 頂点バッファへのポインタ	// 頂点バッファに値をセット
	if (FAILED(m_pD3DVertexBuffer->Lock(0, 0, (LPVOID*)&VBuffer, 0))) 
	{		
...
void c_Primitive::DrawPrimitiveAntialiasing(void)
{
...
void c_Primitive::DrawPrimitive(void)
{
	// 透過色(抜き色)を可能にする
...
void c_Primitive::DrawPrimitiveOtherTexture(LPDIRECT3DTEXTURE9 pD3DTexture)
{
	// 透過色(抜き色)を可能にする
	g_pD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);	// αブレンド可
	g_pD3DDevice->SetRenderState(D3DRS_SRCBLEND,  D3DBLEND_SRCALPHA);
...
void c_Primitive::SetTexRecUVFrame(int nFrameX,int nFrameY)
{
	m_nRecFrameU = nFrameX;
	m_nRecFrameV = nFrameY;
}
void c_Primitive::SetSize(D3DXVECTOR3 size)
{
	m_vSize = size;
}
D3DXMATRIX	c_Primitive::GetMatrix(void)
{
	return m_mMatrix;
}
D3DXVECTOR3	c_Primitive::GetPos(void)
{
	return m_vPos;
}
D3DXVECTOR3	c_Primitive::GetSize(void)
{
	return m_vSize;
}
void c_Primitive::SetTextureUV(int V,int U,int Frame)
{
	if(V < MAX_V_FRAME)
	{
		if(V > m_nMaxFrameV)
