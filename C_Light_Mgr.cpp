...
void c_Light_Mgr::Init_LightMgr(void)
{
	...
	//m_pLight_Mgr->Create_SpotLight(Num,D3DXVECTOR3(0.0f,0.0f,0.0f),D3DXVECTOR3(0.0f,0.0f,-3.0f));
	int NumberSys = *m_pPointList[*Num];
	//m_pLight_Mgr->m_pSysLight[NumberSys]->m_bLookAtTarget = true;
...
HRESULT c_Light_Mgr::Create_PointLight(int* Number)
{
	HRESULT hr;
	int Num;
	if(CheckLightIntList(&Num) == S_FALSE)	//まずライト配列の中に空き状態を確認
		return hr;
	else	
	{
		//ポイントライト配列の空き状態を確認
